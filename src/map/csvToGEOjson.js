...
export default function csvToGEOjson(spec, data) {
  ...
  const longProp =
    (spec['geo-columns'] && spec['geo-columns'][0]) || 'Longitude';
  ...
  for (let d = 0; d < data.length; d += 1) {
    ...
    if (gpsProp && datum[gpsProp] && Array.isArray(datum[gpsProp])) {
      properties[longProp] =
        Number(properties[longProp]) || Number(datum[gpsProp][1]);
      properties[latProp] =
        Number(properties[latProp]) || Number(datum[gpsProp][0]);
    } else if (
      gpsProp &&
      datum[gpsProp] &&
      typeof datum[gpsProp] === 'string'
    ) {
    ...
    features.push({
      ...
      geometry: {
        type: featureType,
        coordinates:
          gpsProp && datum[gpsProp] && Array.isArray(datum[gpsProp])
